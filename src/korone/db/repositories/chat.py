...
async def delete_user_in_group(user_id: int, group: ChatModel) -> UserInGroupModel | None:
    if user := await get_by_chat_id(user_id):
        return await ensure_delete(user, group)
    return None
...
async def upsert_user(user: User) -> ChatModel:
    return await _upsert(user.id, _user_data(user))


async def upsert_group(chat: Chat) -> ChatModel:
    return await _upsert(chat.id, _group_data(chat))


async def _upsert(chat_id: int, data: ChatData) -> ChatModel:
    async with _upsert_lock, session_scope() as session:
