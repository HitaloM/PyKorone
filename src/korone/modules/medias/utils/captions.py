...
def _resolve_author_name(author_name: object, provider: type[MediaProvider]) -> str:
    return author_name if isinstance(author_name, str) and author_name else provider.name


def _resolve_author_handle(author_handle: object, provider: type[MediaProvider]) -> str:
    return author_handle if isinstance(author_handle, str) and author_handle else provider.name.casefold()


def _caption_title(author_name: str, author_handle: str, provider: type[MediaProvider]) -> Text:
