...
DEFAULT_DELIMITERS = {"**": "bold", "__": "italic", "~~": "strikethrough", "++": "underline", "`": "code", "```": "pre"}
...
def extract_markdown_entities(
    ...
    *,
    ...
    delim_re = re.compile(
        "|".join(f"({re.escape(k)})" for k in sorted(delimiters, key=len, reverse=True))  # type: ignore[literal-required]
    )
    ...
    while i < len(text):  # noqa: PLR1702
        ...
        if m:
            ...
            if end != -1:
                ...
                text = "".join((text[:i], text[i + len(delim) : end], text[end + len(delim) :]))
            ...
            if m:
                ...
                result.append(
                    MessageEntity(
                        type="text_link", offset=m.start(), length=len(m.group(1)), url=del_surrogate(m.group(2))
                    )
                )
                i += len(m.group(1))
                continue
