...
async def lfm_command(client: Client, message: Message) -> None:
    ...
    last_played, track_info = lastfm_data

    if not last_played or not track_info:
        return

    user_link = name_with_link(name=str(message.from_user.first_name), username=last_fm_user)
    text = build_response_text(user_link, last_played, track_info)
...
def build_response_text(user_link: str, last_played, track_info) -> str:
