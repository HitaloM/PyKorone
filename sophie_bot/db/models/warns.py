...
class WarnSettingsModel(Document):
    ...
    async def get_or_create(chat_iid: PydanticObjectId) -> WarnSettingsModel:
        ...
        if settings := await WarnSettingsModel.find_one(WarnSettingsModel.chat.iid == chat_iid):
            return settings
...
class WarnModel(Document):
    ...
    async def get_user_warns(chat_iid: PydanticObjectId, user_id: int) -> list[WarnModel]:
        return (
            await WarnModel.find(WarnModel.chat.iid == chat_iid, WarnModel.user_id == user_id)
            .sort(WarnModel.date)
            .to_list()
        )
    ...
    async def count_user_warns(chat_iid: PydanticObjectId, user_id: int) -> int:
        return await WarnModel.find(WarnModel.chat.iid == chat_iid, WarnModel.user_id == user_id).count()
