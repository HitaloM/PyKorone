...
class LocalizationMiddleware(I18nMiddleware):
    async def get_locale(self, event: TelegramObject, data: dict[str, Any]) -> Optional[str]:
        ...
        locale: Optional[str] = await cache_get_locale_name(chat_in_db.id) or None  # type: ignore
