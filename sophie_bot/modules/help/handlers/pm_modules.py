    ...
    async def handle(self) -> Any:
        buttons = InlineKeyboardBuilder().add(
            ...
            *(InlineKeyboardButton(
                ...
                callback_data=PMHelpModule(module_name=module_name).pack()
            ) for module_name, module in HELP_MODULES.items() if not module.exclude_public),
            width=2
        )
...
class PMModuleHelp(CallbackQueryHandler):
    async def handle(self) -> Any:
        ...
        doc += Section(
            format_cmds(list(filter(lambda x: not x.only_admin, cmds))),
            title='Commands'
        )

        doc += Section(
            format_cmds(list(filter(lambda x: x.only_admin, cmds))),
            title='Only admins'
        )
