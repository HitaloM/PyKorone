...
# Welcome Security
class WlcmSecConfigProc(CallbackData, prefix="wlcm_sec_proc"):
    chat_id: int
    user_id: int
    level: str


class WlcmSecConfigCancel(CallbackData, prefix="wlcm_sec_cancel"):
    user_id: int
    level: str
...
async def welcome_security(message: Message, chat, strings):
    ...
    await message.reply(
        strings["ask_for_time_customization"].format(
            time=format_timedelta(convert_time("1h"), locale=strings["language_info"]["babel"])
        ),
    ...
    try:
        ...
        await message.reply(
            strings["welcomesecurity_enabled:customized_time"].format(
                chat_name=chat["chat_title"],
                level=level,
                time=format_timedelta(con_time, locale=strings["language_info"]["babel"]),
            )
        )
    finally:
        await state.clear()
...
async def reset_security_note(message: Message, chat, strings):
    ...
    if (
        await db.greetings.update_one({"chat_id": chat_id}, {"$unset": {"security_note": 1}}, upsert=True)
    ).modified_count > 0:
        await get_greetings_data.reset_cache(chat_id)
        text = strings["security_note_updated"]
    else:
...
async def welcome_security_handler(message: Message, strings):
    ...
    kwargs["reply_to"] = (
        None if "clean_service" in db_item and db_item["clean_service"]["enabled"] is True else message.message_id
    )
...
async def ws_redirecter(message: Message, strings, **kwargs):
    ...
    url = f"https://t.me/{CONFIG.username}?start=ws_{chat_id}_{called_user_id}_{message.message.message_id}"
    if not called_user_id == real_user_id:
        ...
        if not redis.exists(f"welcome_security_users:{called_user_id}:{chat_id}"):
            ...
            url = f"https://t.me/{CONFIG.username}?start=ws_{chat_id}_{called_user_id}_{message.message.message_id}_0"
...
async def change_captcha(event, strings, state=None, **kwargs):
    ...
    await message.edit_media(
        InputMediaPhoto(media=BufferedInputFile(img.read(), filename="captcha.png"), caption=text),
        reply_markup=buttons,
    )
...
async def welcome_trigger(message: Message, strings):
    ...
    reply_to = (
        message.message_id if "clean_welcome" in db_item and db_item["clean_welcome"]["enabled"] is not False else None
    )
    text, kwargs = await t_unparse_note_item(message, db_item["note"], chat_id)
...
async def clean_service_trigger(message: Message, strings):
    ...
    if not await check_admin_rights(message, chat_id, CONFIG.bot_id, ["can_delete_messages"]):
