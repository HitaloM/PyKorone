...
from sophie_bot.modules.legacy_modules.utils.restrictions import ban_user, unban_user
...
async def new_fed(message: Message, strings):
    ...
    await message.reply(
        strings["created_fed"].format(name=fed_name, id=data["fed_id"], creator=await get_user_link(user_id))
    )
...
async def join_fed(message: Message, chat, strings):
    ...
    await db.feds.update_one({"_id": fed["_id"]}, {"$addToSet": {"chats": {"$each": [chat_id]}}})
    await get_fed_by_id.reset_cache(fed["fed_id"])
    await message.reply(
        strings["join_fed_success"].format(chat=chat["chat_title"], fed=html.escape(fed["fed_name"], False))
    )
    await fed_post_log(
        fed,
...
async def leave_fed_comm(message: Message, chat, fed, strings):
    ...
    await get_fed_by_id.reset_cache(fed["fed_id"])
    await message.reply(
        strings["leave_fed_success"].format(chat=chat["chat_title"], fed=html.escape(fed["fed_name"], False))
    )
...
async def fed_sub(message: Message, fed, strings):
    ...
    await db.feds.update_one({"_id": fed["_id"]}, {"$addToSet": {"subscribed": {"$each": [fed_id]}}})
    await get_fed_by_id.reset_cache(fed["fed_id"])
    await message.reply(
...
async def fed_unsub(message: Message, fed, strings):
    ...
    if "subscribed" in fed and fed_id not in fed["subscribed"]:
        message.reply(strings["not_subsed"].format(name=html.escape(fed["fed_name"], False), name2=fed2["fed_name"]))
        return

    await db.feds.update_one({"_id": fed["_id"]}, {"$pull": {"subscribed": str(fed_id)}})
    await get_fed_by_id.reset_cache(fed["fed_id"])
    await message.reply(
...
async def promote_to_fed(message: Message, fed, user, text, strings):
    ...
    await db.feds.update_one({"_id": fed["_id"]}, {"$addToSet": {"admins": {"$each": [user["user_id"]]}}})
    await get_fed_by_id.reset_cache(fed["fed_id"])
    await message.reply(
...
async def set_fed_log_chat(message: Message, fed, chat, strings):
    ...
    await fed_post_log(
        ...
        strings["set_log_fed_log"].format(fed_name=html.escape(fed["fed_name"], False), fed_id=fed["fed_id"]),
    )
...
async def unset_fed_log_chat(message: Message, fed, strings):
    ...
    await fed_post_log(
        fed,
        strings["unset_log_fed_log"].format(fed_name=html.escape(fed["fed_name"], False), fed_id=fed["fed_id"]),
    )
...
async def fed_ban_user(message: Message, fed, user, reason, strings):
    ...
    if len(sfeds_list := await get_all_subs_feds_r(fed["fed_id"], [])) > 1:
        ...
        channel_text += strings["fban_subs_fed_log"].format(subs_chats=all_banned_chats_count, feds=len(sfeds_list))
...
async def unfed_ban_user(message: Message, fed, user, text, strings):
    ...
    if len(sfeds_list := await get_all_subs_feds_r(fed["fed_id"], [])) > 1:
        ...
        channel_text += strings["fban_subs_fed_log"].format(subs_chats=all_unbanned_chats_count, feds=len(sfeds_list))
    else:
...
async def del_fed_func(event, strings, callback_data: DelFedCb, **kwargs):
    ...
    async for subscribed_fed in db.feds.find({"subscribed": fed_id}):
        await db.feds.update_one({"_id": subscribed_fed["_id"]}, {"$pull": {"subscribed": fed_id}})
