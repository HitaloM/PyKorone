...
async def get_notes_list(message: Message, strings, chat, keyword=None, pm=False):
    ...
    if len(keyword := get_args_str(message)) > 0 and pm is False:
        await search_notes(keyword)
...
async def search_in_note(message: Message, chat, strings):
    ...
    notes = db.notes.find({"chat_id": chat["chat_id"], "text": {"$regex": request, "$options": "i"}}).sort("names", 1)
...
async def clean_notes_set(message: Message, chat, strings):
    ...
    if arg and arg.lower() in enable:
        ...
        await db.clean_notes.update_one(
            {"chat_id": chat_id},
            {"$set": {"enabled": True, "keep_num": keep_num}},
            upsert=True,
        )
        text = strings["clean_notes_enable"].format(chat_name=chat["chat_title"])
        text += "\n"
