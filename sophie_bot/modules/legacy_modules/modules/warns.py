...
async def warn_func(message: Message, chat, user, text, strings, filter_action=False):
    ...
    buttons = InlineKeyboardMarkup(
        inline_keyboard=[[InlineKeyboardButton(text="⚠️ Remove warn", callback_data="remove_warn_{}".format(warn_id))]]
    )

    if await db.rules.find_one({"chat_id": chat_id}):
        buttons.inline_keyboard.append(
            [InlineKeyboardButton(text="📝 Rules", url=await get_start_link(f"btn_rules_{chat_id}"))]
        )
            ...
            if data is not None:
                if data["mode"] == "tmute":
                    ...
                    text = strings["max_warn_exceeded"].format(
                        user=member,
                        action=(strings["banned"] if data["mode"] == "ban" else strings["muted"]),
                    )
...
async def reset_warn(message: Message, chat, user, strings):
    ...
    if await db.warns.find_one({"chat_id": chat_id, "user_id": user_id}):
        ...
        await message.reply(
            strings["purged_warns"].format(admin=admin_link, num=purged, user=user_link, chat_title=chat_title)
        )
    else:
