...
from stfu_tg import PreformattedHTML, Doc, Section, VList, HList
...
def retrieve_tools_titles(message_history: list[ModelRequest | ModelResponse]) -> list[Element]:
    ...
    tool_parts = [part for part in all_parts
                  if isinstance(part, ToolCallPart) or isinstance(part, ToolReturnPart)]

    # Extract unique tool names that exist in our titles dictionary
    unique_tool_names = {part.tool_name for part in tool_parts
                         if part.tool_name in CHATBOT_TOOLS_TITLES}
...
async def ai_chatbot_reply(
    ...
    header_items = [
        *retrieve_tools_titles(result.message_history),
        HList(divider=', ')
    ]
