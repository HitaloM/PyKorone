...
async def get_parsed_note_list(message: Message, allow_reply_message=True, split_args=1):
    ...
    if message.reply_to_message and allow_reply_message:
        ...
        if get_cmd(message) and get_args_str(message):
            # Remove cmd and arg from message's text
            text = re.sub(rf"[{CONFIG.commands_prefix}]" + get_cmd(message) + r"\s?", "", text, 1)
            if split_args > 0:
    ...
    for raw_button in raw_buttons:
        ...
        if action in BUTTONS.keys():
            ...
            if aio:
                ...
                start_btn = Button.url(name, f"https://t.me/{CONFIG.username}?start=" + string)
            ...
            if argument[0] == "/" and argument[1] == "/":
                argument = argument[2:]
            btn = InlineKeyboardButton(text=name, url=argument) if aio else Button.url(name, argument)
        else:
