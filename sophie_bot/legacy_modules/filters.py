...
async def setup_end(message, chat, strings, state: FSMContext, **kwargs):
    ...
    func = action['setup'][state_data['curr_step']]['finish'] if type(action['setup']) is list else action['setup'][
        'finish']
...
async def delall_filters(message: Message, strings: dict):
    ...
    buttons = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(
            text=strings['confirm_yes'], callback_data=FilterDelallYesCb(chat_id=message.chat.id).pack()
        ),
        InlineKeyboardButton(
            text=strings['confirm_no'], callback_data="filter_delall_no_cb"
        )
    ]])
