...
async def welcome_security(message, chat, strings):
    ...
    buttons = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(
            text=strings["no_btn"], callback_data=WlcmSecConfigCancel(
                user_id=message.from_user.id, level=level
            ).pack()
        ),
        InlineKeyboardButton(
            text=strings["yes_btn"], callback_data=WlcmSecConfigProc(
...
async def change_captcha(event, strings, state=None, **kwargs):
    ...
    async with state.proxy() as data:
        ...
        if regen_num > 3:
            ...
            await message.edit_media(InputMediaPhoto(media=BufferedInputFile(
                img.read(), filename='captcha.png'), caption=text))
            return
    ...
    buttons = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(
            text=strings['regen_captcha_btn'],
            callback_data='regen_captcha'
        )
    ]])

    await message.edit_media(InputMediaPhoto(media=BufferedInputFile(
        img.read(), filename='captcha.png'), caption=text), reply_markup=buttons)
