...
async def select_lang_keyboard(message, strings, edit=False):
    ...
    markup.inline_keyboard.append([InlineKeyboardButton(
        text=strings['crowdin_btn'], url='https://crowdin.com/project/sophiebot')
    ])
    if edit:
...
async def change_lang(message, lang, e=False, back_btn=False):
    ...
    if 'translators' in lang_info:
        markup.inline_keyboard.append([InlineKeyboardButton(
            text=strings['see_translators'], callback_data=TranslatorsLangCb(lang=lang).pack()
        )])

    if back_btn == 'True':
        # Callback_data converts boolean to str
        markup.inline_keyboard.append([InlineKeyboardButton(
            text=strings['back'], callback_data='go_to_start'
        )])
