...
4. Add the handler to `manifest.handlers` in `medias/__init__.py`.
...
- Use shared HTTP session from `HTTPClient.get_session()`.
...
- Module handler wiring through `medias/__init__.py` `ModuleManifest(..., handlers=(...))`.
...
- Build media list via `download_media(...)` unless preserving an existing platform-specific path.
