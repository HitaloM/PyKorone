...
async def add_filter(chat_id: int, handler: str, raw_data: str, file_id: int, filter_type: str):
    await conn.execute(
...
async def update_filter(chat_id: int, handler: str, data: str, file_id: int, filter_type: str):
    await conn.execute(
        "UPDATE filters SET data = ?, file_id = ?, filter_type = ? WHERE chat_id = ? \
AND handler = ?",
...
async def remove_filter(chat_id: int, handler: str):
    await conn.execute("DELETE from filters WHERE chat_id = ? AND handler = ?", (chat_id, handler))
    await conn.commit()
